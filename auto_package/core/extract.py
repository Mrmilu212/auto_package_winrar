...
def find_winrar_exe() -> Path | None:
    ...
    for root in roots:
        ...
        if candidate.is_file():
            logger.debug('发现 WinRAR.exe: %s', candidate)
            return candidate
    # PATH
    ...
    w = which("WinRAR.exe")
    if w:
        logger.debug('发现 WinRAR.exe: %s', w)
        return Path(w)
    logger.warning('未找到 WinRAR.exe')
    return None
...
def _winrar_extract(
    ...
    *, 
    ...
    logger.debug('执行解压命令: %s', ' '.join(cmd))
    ...
    while True:
        if cancel_ev is not None and cancel_ev.is_set():
            logger.info('解压已取消: %s', archive_path)
            try:
                p.terminate()
                p.wait(timeout=1.5)
    ...
    if rc != 0:
        ...
        logger.error('解压失败: %s, 错误: %s', archive_path, error_msg)
        return False, error_msg
    logger.info('解压完成: %s, 耗时: %.2fs', archive_path, time.monotonic() - start)
...
def _try_extract_with_formats(
    ...
    *, 
    ...
    if suffix in _ARCHIVE_EXTS:
        logger.debug('识别格式: %s，直接解压', suffix)
    ...
    logger.debug('尝试去伪装: %s -> %s', file_path, temp_path)
    try:
    ...
    logger.debug('清理临时文件: %s', temp_path)
    
    if ok:
        logger.info('去伪装解压成功: %s', file_path)
        return True, msg
    
    logger.debug('去伪装解压失败: %s', file_path)
    return False, "不是压缩包或解压失败"
...
def _check_and_extract_recursive(
    ...
    *, 
    ...
    3. 只有文件夹 → 递归检查每个文件夹
    """
    logger.debug('递归解压第 %d 层: %s', depth, current_dir)
    ...
    if has_files and has_folders:
        logger.info('混合内容，判定为最终文件: %s', current_dir)
    ...
    if has_files and not has_folders:
        ...
        if non_archive_count == 1:
            ...
            if non_archive_file:
                logger.debug('单个非压缩文件，尝试zip解压: %s', non_archive_file)
                # 仅尝试改为zip解压
                temp_extract = current_dir / f"_extract_{_random_token(4)}"
                try:
                    temp_extract.mkdir(exist_ok=False)
                    logger.debug('创建临时解压目录: %s', temp_extract)
                except Exception as e:
                    logger.debug('无法创建临时目录: %s', e)
                ...
                try:
                    ...
                    logger.debug('创建临时zip文件: %s', temp_zip)
                except Exception as e:
                    logger.debug('无法复制文件: %s', e)
                ...
                logger.debug('清理临时zip文件: %s', temp_zip)
                
                if ok:
                    # 删除原文件
                    _safe_unlink(non_archive_file)
                    logger.debug('删除原文件: %s', non_archive_file)
                    # 递归检查解压后的内容
                    logger.debug('递归检查解压后的内容: %s', temp_extract)
                    ...
                    # 解压失败，删除临时目录，判定为最终文件
                    _safe_rmtree(temp_extract)
                    logger.debug('解压失败，清理临时目录: %s', temp_extract)
                    logger.info('单个非压缩文件解压失败，判定为最终文件: %s', non_archive_file)
        ...
        for file_item in list(current_dir.iterdir()):
            ...
            logger.debug('尝试解压压缩文件: %s', file_item)
            # 创建临时解压目录
            temp_extract = current_dir / f"_extract_{_random_token(4)}"
            try:
                temp_extract.mkdir(exist_ok=False)
            ...
            if ok:
                ...
                logger.debug('删除原压缩包: %s', file_item)
                # 递归检查解压后的内容
                logger.debug('递归检查解压后的内容: %s', temp_extract)
                ok2, msg2 = _check_and_extract_recursive(
                    winrar_exe, temp_extract, out_dir,
        ...
        if not any_success:
            logger.info('所有文件解压失败，判定为最终文件: %s', current_dir)
            return True, "所有文件解压失败，判定为最终文件"
        logger.info('递归解压完成: %s', current_dir)
        return True, "递归解压完成"
    ...
    if has_folders and not has_files:
        for folder_item in list(current_dir.iterdir()):
            if not folder_item.is_dir():
                continue
            logger.debug('递归检查文件夹: %s', folder_item)
            ...
            if not ok:
                return False, msg
        logger.info('文件夹递归检查完成: %s', current_dir)
...
def run_auto_extract(
    ...
    *, 
    ...
    try:
        ...
        if phase_cb:
            phase_cb(1, 3)
        logger.debug('进入阶段 1/3: 准备解压')

        # 创建解压目录
        extract_dir = Path(work_dir) / "extract"
        extract_dir.mkdir(exist_ok=True)
        logger.debug('创建解压目录: %s', extract_dir)
        ...
        if not ok:
            logger.error('解压失败: %s', msg)
            return False, f"解压失败: {msg}", None

        # 阶段 2：递归检查并解压
        if phase_cb:
            phase_cb(2, 3)
        logger.debug('进入阶段 2/3: 递归检查并解压')
        ok, msg = _check_and_extract_recursive(
        ...
        if not ok:
            logger.error('递归检查失败: %s', msg)
            return False, msg, None

        # 阶段 3：移动到目标目录
        if phase_cb:
            phase_cb(3, 3)
        logger.debug('进入阶段 3/3: 移动到目标目录')
        ...
        logger.error('解压过程出错: %s', e)
        return False, f"解压过程出错: {e}", None
    finally:
        # 确保临时目录被删除
        _safe_rmtree(work_dir)
        logger.debug('清理临时工作目录: %s', work_dir)